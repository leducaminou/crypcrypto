...
interface WithdrawalFiltersProps {
  selectedStatus: TransactionStatusType | 'all';
  onStatusChange: (status: TransactionStatusType | 'all') => void;
}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <div>
          ...
          <select
            ...
            onChange={(e) => onStatusChange(e.target.value as TransactionStatusType | 'all')}
          ...
          >
            ...
            <option value={TransactionStatus.COMPLETED}>
              {getStatusTranslation(TransactionStatus.COMPLETED)}
            </option>
            <option value={TransactionStatus.PENDING}>
              {getStatusTranslation(TransactionStatus.PENDING)}
            </option>
            <option value={TransactionStatus.FAILED}>
              {getStatusTranslation(TransactionStatus.FAILED)}
            </option>
            <option value={TransactionStatus.CANCELLED}>
              {getStatusTranslation(TransactionStatus.CANCELLED)}
            </option>
          </select>
        </div>
